...
7. **Child prompts are fixed templates — no inline reasoning.** The parent must
   ...
   cert id, question range, artifact path, prior concepts path). The parent
   must not draft or reason about the prompt text inline. Inner-monologue
   artifacts ("if typo? No, use X", "wait, should this be Y?") in a child
   prompt indicate the parent is reasoning in the prompt body — this is a
   disqualifying failure. Construct the prompt, then send it; do not narrate
...
8. **Every child prompt must include the runbook read instruction.** Each child
   ...
   `automation/prompts/generate-exams.md` (Step 4, Ledger format, Ledger
   quality gates, Known failure modes, Content rules) before acting. Do not
   summarise the quality gates inline as a replacement for reading the runbook.
